...
class BridgeReconstructor:
    ...
    def reconstructIt(self):
        ...
        for _i in range(len(self.bridges)):
            ...
            for i in range(len(bridge.shape.points) - 1):
                ...
                test = self.findPointsUnderBridge(point_a, point_b, width, depth)
                point_b = [point_c[0] + unit_vector[0] * magnitude, point_c[1] + unit_vector[1] * magnitude]
                # Search along the other side of bridge
                test2 = self.findPointsUnderBridge(point_c, point_b, width, depth)
                ...
                for point in test:
                    for point2 in test2:
                        if abs(point[2] - point2[2]) < 10 and 0 < self.distance(point, point2) < width + 150:
    ...
    def distance(self, point_a, point_b):
        if len(point_a) != len(point_b):
            print("Wrong arguments for distance")
            return False
        else:
            if len(point_a) > 2:
    ...
    def findPointsUnderBridge(self, point_a, point_b, width, depth):
        tmp1 = [pnt for pnt in self.laz.laz.points if pnt[2] < depth and self.isCloseToVector(point_a, point_b, pnt)]
